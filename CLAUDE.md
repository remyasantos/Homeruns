...
- **Matchup score**: strictly historical hitter-vs-pitcher rating — `zone_fit_norm(0-30) + xwobac_norm(0-20) + ceiling*0.50`, `None` if any input isn't real. Deliberately excludes park/weather. Weighting is evidence-based (not a guess): regressed a reference dashboard's own real matchup_score against 548 real joined batter+opposing-pitcher records across two separate days — Ceiling alone explains R²=0.62 of the variance (the dominant real driver), Zone Fit/xwOBAc add only marginal lift on top, and the opposing pitcher's own quality score added ~zero once those were included (dropped entirely). Still an approximation (R²~0.62, not exact like `compute_khr()`) — see that function's docstring for the full derivation and why an exact match wasn't reachable.
